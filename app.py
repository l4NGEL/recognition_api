...
class FaceRecognitionAPI:
    ...
    def compare_embeddings_batch(self, query_embedding, known_embeddings, threshold=0.8):
        """
        ...
        # Euclidean mesafe hesaplama (daha hızlı)
    ...
    def add_user(self, name, images_base64, id_no=None, birth_date=None):
        try:
            ...
            for idx, img_b64 in enumerate(images_base64):
                ...
                # Yüzü kes (daha geniş al - siyah kenarları önlemek için)
                margin = int(min(w, h) * 0.25)  # %25 margin
                x1 = max(0, x - margin)
                y1 = max(0, y - margin)
                ...
                # Farklı rotasyonlarda fotoğraflar oluştur ve kaydet
            ...
            for test_embedding in embeddings_to_test:
                if self.known_embeddings_array is not None and len(self.known_embeddings_array) > 0:
                    current_match_idx, current_distance = self.compare_embeddings_batch(
                        test_embedding, self.known_embeddings_array, threshold=0.8
                    )
